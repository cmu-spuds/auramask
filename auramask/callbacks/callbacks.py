...
class ImageCallback(TensorBoard):
        ...
        with self._train_writer.as_default():
            ...
            if os.getenv("SLURM_JOB_NAME") and os.getenv("SLURM_ARRAY_TASK_ID"):
                hp.hparams(
                    tmp_hparams,
                    trial_id="%s-%s"
                    % (os.environ["SLURM_JOB_NAME"], os.environ["SLURM_ARRAY_TASK_ID"]),
                )
            else:
            ...
            tf.summary.image(
                "Epoch/Validation",
                tf.concat([self.v_sample[0], self.v_sample[1]], axis=2),
                max_outputs=2,
                step=-1,
            )
            tf.summary.image(
                "Epoch/Training",
                tf.concat([self.t_sample[0], self.t_sample[1]], axis=2),
                max_outputs=2,
                step=-1,
            )
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        _should_checkpoint = (
            self.model_checkpoint_frequency
            and epoch > 0
            and (epoch) % self.model_checkpoint_frequency == 0
        )
        if _should_checkpoint:
            self.model.save_weights(
                os.path.join(self.log_dir, "%04d-model.weights.h5" % (epoch)),
                overwrite=True,
            )
        with self._train_writer.as_default():
            with tf.name_scope("Epoch"):
                _should_update_img = (
                    self.image_frequency and (epoch) % self.image_frequency == 0
                )
                _should_update_mask = (
                    self.mask_frequency and (epoch) % self.mask_frequency == 0
                )
                if _should_update_img or _should_update_mask:
                    y, mask = self.model(
                        tf.concat([self.t_sample[0], self.v_sample[0]], axis=0)
                    )
                    if _should_update_img:
                        tf.summary.image(
                            "Training",
                            tf.concat([y[:5], self.t_sample[1][:5]], axis=2),
                            max_outputs=2,
                            step=epoch,
                        )
                        tf.summary.image(
                            "Validation",
                            tf.concat([y[-5:], self.v_sample[1][:5]], axis=2),
                            max_outputs=2,
                            step=epoch,
                        )
