from keras import Optimizer, ops, random
    ...
    def compute_gradients(
        ...
        def proj_grad(grad_task):
            for k in range(num_tasks):
                inner_product = ops.sum(grad_task * grads_task[k])
                proj_direction = inner_product / ops.sum(grads_task[k] * grads_task[k])
                grad_task = grad_task - ops.minimum(proj_direction, 0.0) * grads_task[k]
            return grad_task
