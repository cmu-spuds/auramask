...
class ColorTransformMethods(unittest.TestCase):
    ...
    def test_to_yuv(self):
        ...
        shifted_to_ = tf.subtract(to_, [0.,0.5,0.5])
        tf_to_ = tf.image.rgb_to_yuv(self._test_img)

        # Test Y is in [0, 1]
        tf.debugging.assert_less_equal(tf.reduce_max(shifted_to_[:,:,0]), 1.)
        tf.debugging.assert_greater_equal(tf.reduce_min(shifted_to_[:,:,0]), 0.)
        ...
        self.assertGreaterEqual(tf.reduce_min(to_), 0.0)
        self.assertEqual(to_.shape, self._test_img.shape)
        tf.debugging.assert_near(tf.reduce_mean(tf_to_), tf.reduce_mean(shifted_to_), atol=self.atol, rtol=self.rtol)
    ...
    def test_to_and_from_yuv(self):
        ...
        self.assertGreaterEqual(tf.reduce_min(processed_), 0.0)
        self.assertEqual(processed_.shape, self._test_img.shape)
        tf.debugging.assert_near(tf.reduce_mean(processed_), tf.reduce_mean(self._test_img), atol=self.atol, rtol=self.rtol)
    ...
    def test_to_yuv_batch(self):
        ...
        self.assertGreaterEqual(tf.reduce_min(to_), 0.0)
        self.assertEqual(to_.shape, self._test_batch_imgs.shape)
        tf.debugging.assert_near(tf.reduce_mean(shifted_to_, axis=[1,2,3]), tf.reduce_mean(tf_to_, axis=[1,2,3]), summarize=5, atol=self.atol, rtol=self.rtol)
    ...
    def test_to_and_from_yuv_batch(self):
        ...
        self.assertGreaterEqual(tf.reduce_min(processed_), 0.0)
        self.assertEqual(processed_.shape, self._test_batch_imgs.shape)
        tf.debugging.assert_near(tf.reduce_mean(processed_, axis=[1,2,3]), tf.reduce_mean(self._test_batch_imgs, axis=[1,2,3]), summarize=5, atol=self.atol, rtol=self.rtol)
